    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> AddInstructor(CreateInstructorViewModel model)
        {
            if (ModelState.IsValid) 
            {
                var instructor = new ApplicationUser()
                {
                    ...
                    ImagePath = DocumentSetting.UploadFile(model.Image, "User"),
                ...
                if (!result.Succeeded) 
                {
                    DocumentSetting.DeleteFile("User", instructor.ImagePath);
        ...
        public async Task<IActionResult> Update(UpdateInstructorViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (hasImage)
                {
					DocumentSetting.DeleteFile("User", model.ImagePath);// delete old image
					instructor.ImagePath = DocumentSetting.UploadFile(model.Image, "User");// add new image
				}
